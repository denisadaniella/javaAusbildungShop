...
create table if not exists  customer (id integer generated by default as identity, firstname varchar(255), lastname varchar(255), userid integer not null, primary key (id));
...
create table if not exists  users (id integer generated by default as identity, username varchar(255) not null, password varchar(255) not null, primary key (id));
create table if not exists  userrole (id integer generated by default as identity, role varchar(255) not null, userid integer not null, primary key (id));
...
alter table customer add constraint FK_customer_user foreign key (userid) references users;
alter table userrole add constraint FK_userrole_user foreign key (userid) references users;
